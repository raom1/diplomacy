...
from policies import test_policy, test_build_policy
...
def initialize_game_assets(self):
...
def play_one_round(self, policy, units_df, territories_df, o = None, retreat = False, season = None, human = False, model = None, targets = None, move_resolver = None):
    begin = time()
    if not retreat:
        ...
        if len(need_retreat) > 0:
            if human:
                ...
                for c in ['france','italy','england','russia','germany','austria','turkey']:
                    orders_list, _, _, _, _ = move_resolver.make_orders(units_df, territories_df, test_policy, loss_fn, sea_model=sea_model, land_model=land_model, allies=allies)
...
def check_win(self, territories_df):
    begin = time()
	...
	for _ in range(5): #100
	    ...
	    for _ in range(num_trials):
	        ...
	        while winner is None and rounds < 350:
	            ...
	            all_orders, all_out_probs, all_grads, targets_list, all_model_vers = move_resolver.make_orders(units_df, territories_df, test_policy,
	            																				 loss_fn,
	            																				 sea_model=sea_model,
	            																				 land_model=land_model,
	            																				 allies=allies) #test_policy
	            ...
	            if season == 'fall':
	                ...
	                units_df, builds_output_dict = builds_maker.make_builds(units_df, territories_df, build_loss_fn, policy = test_build_policy, model = builds_model, unit_rewards = total_build_rewards_grads, active_country = active_country, rewards_calculator = rewards_calculator) #test_build_policy
